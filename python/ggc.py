...
def get_landmark_data(f):
	...
	for row in csv.DictReader(f):  
		...
		if row['開放時間'].find('\n') == -1:
			#landmark_str += "{\"name\":\"" + row['景點中文名稱'] + "\"," + "\"type\":\"landmark\"," + "\"time\":\"" + row['開放時間']                                               + "\"," + "\"lat\":\"" + row['Y坐標'] + "\"," + "\"lng\":\"" + row['X坐標'] + "\"," + "\"address\":\"" + row['地址'] + "\"},"
			None
		else:
			landmark_str += "{\"name\":\"" + row['景點中文名稱'] + "\"," + "\"type\":\"landmark\"," + "\"time\":\"" + row['開放時間'].split('\n')[0] + row['開放時間'].split('\n')[1] + "\"," + "\"lat\":\"" + row['Y坐標'] + "\"," + "\"lng\":\"" + row['X坐標'] + "\"," + "\"address\":\"" + row['地址'] + "\"},"
...
#total += get_landmark_data(landmark)

print(json.JSONEncoder().encode(total))
